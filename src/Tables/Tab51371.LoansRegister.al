...
Table 51371 "Loans Register"
{
    ...
    fields
    {
                    ...
                    if LoanType.Get("Loan Product Type") then begin
                        if Source = Source::BOSA then begin
                            ...
                            if ObjAccountLedger.Find('-') then begin
                                ...
                                AverageDep := 10 * (-1 * (ObjAccountLedger."Amount Posted" / 3));
        ...
        {
            CalcFormula = sum("Cust. Ledger Entry"."Amount Posted" where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Insurance Charge"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Posting Date" = field("Date filter")));
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = sum("Cust. Ledger Entry"."Amount Posted" where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Loan Insurance Paid" | "Insurance Charge"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Posting Date" = field("Date filter")));
            FieldClass = FlowField;
        }
    ...
    begin
        ...
        if InstalmentDays <> 0 then begin
